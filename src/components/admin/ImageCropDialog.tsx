...
export function ImageCropDialog({ file, aspect, onCancel, onConfirm, title = "Crop image" }: Props) {
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-2 pt-2">
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            ...
            <Slider value={[zoom]} min={1} max={4} step={0.05} onValueChange={(v) => setZoom(v[0] ?? 1)} />
            <span className="w-10 text-right tabular-nums">{zoom.toFixed(2)}×</span>
          </div>
          <p className="text-xs text-muted-foreground">
            Drag to reposition. Aspect is locked to {aspect.toFixed(2)}:1 for this field.
          </p>
        </div>
        <DialogFooter>
